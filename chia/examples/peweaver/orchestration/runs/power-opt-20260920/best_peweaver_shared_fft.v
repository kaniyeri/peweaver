...
module peweaver_shared_fft (
    ...
    // Stage 3: Radix-2 SDF (active only in 128-pt mode, isolated in 64-pt mode)
    wire        su3_di_en = mode_q ? su2_do_en : 1'b0;
    wire [15:0] su3_di_re = mode_q ? su2_do_re : 16'd0;
    wire [15:0] su3_di_im = mode_q ? su2_do_im : 16'd0;
    ...
    // Final Output Multiplexing (driven strictly by latched mode_q)
    assign do_en = mode_q ? su3_do_en : su2_do_en;
    assign do_re = mode_q ? su3_do_re : su2_do_re;
    assign do_im = mode_q ? su3_do_im : su2_do_im;
...
module SdfUnitShared #(
    ...
    input               clock,
    input               reset,
    input               mode,   // latched mode_q: 0 = 64-pt, 1 = 128-pt
    input               di_en,
    ...
    // In 64-pt mode, rom_addr is forced to 0 for stage 2 to keep twiddles zeroed
    assign rom_addr = bypass_mult ? 7'd0 : (mode ? tw_addr_128 : {tw_addr_64, 1'b0});
    ...
    // Direct connections from registers to multiplier inputs (no intervening muxes)
