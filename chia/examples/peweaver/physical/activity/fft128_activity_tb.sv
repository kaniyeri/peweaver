//----------------------------------------------------------------------
// PEWeaver FFT128 PPA activity + golden testbench (Phase 2 physical adaptation).
//
// PEWeaver-owned. One testbench serves both activity modes:
//   * RTL mode  : compiles rtl/peweaver_ppa_fft128.v + reference fixture RTL
...
//   phase 1: reset-separated frame A (vectors/input4.txt, 128 samples); outputs drained;
...
//             exactly 384 consecutive samples (frames input4, input5,
//             input4) with no reset and no gap inside the burst; the
//             collector captures exactly 384 consecutive do_en outputs
//             starting at the first do_en. Expected output windows:
//             output4, output5, output4 (128 samples each).
//   phase 3: final idle drain
//
// Golden contract: captured emission sample j of window w must equal golden
// frame w at natural bin bitrev7(j) (bit-reversed emission order).
//
// VCD modes:
...
module peweaver_fft128_activity_tb;

    localparam int FRAME = 128;
    localparam int DRAIN = 256;
    localparam int STREAM_FRAMES = 3;
    localparam int BURST_SAMPLES = STREAM_FRAMES * FRAME;
    localparam int BURST_CAP = BURST_SAMPLES + 160;  // burst outputs + latency margin
    ...
    // Stream one 128-sample frame of imem at frame_base; returns after the
    ...
    // sample j is natural bin bitrev7(j)).
    task automatic compare_window(input string golden_path, input string label);
        ...
        begin
            ...
            for (int n = 0; n < FRAME; n++) begin
                if (win_re[bitrev7(n)] !== gmem[2*n]
                    || win_im[bitrev7(n)] !== gmem[2*n + 1]) begin
                    mismatches++;
                    if (mismatches <= 4)
                        $display("[FAIL] %s bin %0d got (%h,%h) expected (%h,%h)",
                                 label, n, win_re[bitrev7(n)], win_im[bitrev7(n)],
                                 gmem[2*n], gmem[2*n + 1]);
                end
            end
    ...
    initial begin : stimulus
        ...
        if ($value$plusargs("VCD=%s", vcd_path) && !stream_only) begin
            ...
            $dumpfile(vcd_path);
            $dumpvars(0, peweaver_fft128_activity_tb);
        end
        ...
        begin : burst_compare
            ...
            for (int w = 0; w < STREAM_FRAMES; w++) begin
                ...
                mismatches = 0;
                for (int n = 0; n < FRAME; n++) begin
                    if (burst_re[w*FRAME + bitrev7(n)] !== gmem[2*n]
                        || burst_im[w*FRAME + bitrev7(n)] !== gmem[2*n + 1]) begin
                        mismatches++;
                        if (mismatches <= 4)
                            $display("[FAIL] burst w%0d bin %0d got (%h,%h) expected (%h,%h)",
                                     w, n, burst_re[w*FRAME + bitrev7(n)], burst_im[w*FRAME + bitrev7(n)],
                                     gmem[2*n], gmem[2*n + 1]);
                    end
                end
                if (mismatches != 0) begin
