// PEWeaver ungated dual-core FFT reference activity and golden testbench.
// Schedule is an EXACT mirror of shared_activity_tb.sv (same task bodies,
// same timing, same vectors) so power windows are matched by construction.
// +STREAM_MODE=64/128 selects the workload for a +STREAM_ONLY power trace.
`timescale 1ns/1ns

module peweaver_dual_b_activity_tb;
    ...
    initial begin : stimulus
        ...
        if ($value$plusargs("VCD=%s", vcd_path) && !stream_only) begin
            $dumpfile(vcd_path); $dumpvars(0, peweaver_dual_b_activity_tb);
        end
