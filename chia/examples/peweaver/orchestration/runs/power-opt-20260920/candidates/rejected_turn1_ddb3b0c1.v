...
module peweaver_shared_fft (
    ...
    input       [15:0]  di_re,  // Input Data (Real)
    input       [15:0]  di_im,  // Input Data (Imag)
    output              do_en,  // Output Data Enable
    output      [15:0]  do_re,  // Output Data (Real)
    output      [15:0]  do_im   // Output Data (Imag)
);

    //------------------------------------------------------------------
    //  Mode Latching and Frame Tracking
    //------------------------------------------------------------------
    reg [7:0] outstanding;
    reg       mode_q;

    wire idle = (outstanding == 8'd0);
    wire active_mode = idle ? mode : mode_q;
    ...
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            if (di_en && !do_en) begin
                outstanding <= outstanding + 1'b1;
            end else if (!di_en && do_en) begin
                outstanding <= outstanding - 1'b1;
            end
        end
    end
    ...
    // Stage 0: Radix-2^2 SDF (M=128 in 128-pt, M=64 in 64-pt)
    SdfUnitShared #(.STAGE(0), .WIDTH(16)) SU0 (
        .clock  (clock      ),
        .reset  (reset      ),
        .mode   (active_mode),
        .di_en  (di_en      ),
    ...
    // Stage 1: Radix-2^2 SDF (M=32 in 128-pt, M=16 in 64-pt)
    SdfUnitShared #(.STAGE(1), .WIDTH(16)) SU1 (
        .clock  (clock      ),
        .reset  (reset      ),
        .mode   (active_mode),
        .di_en  (su0_do_en  ),
    ...
    // Stage 2: Radix-2^2 SDF (M=8 in 128-pt, M=4 in 64-pt)
    SdfUnitShared #(.STAGE(2), .WIDTH(16)) SU2 (
        .clock  (clock      ),
        .reset  (reset      ),
        .mode   (active_mode),
        .di_en  (su1_do_en  ),
    ...
    // Stage 3: Radix-2 SDF (active only in 128-pt mode, bypassed in 64-pt mode)
    wire        su3_di_en = active_mode ? su2_do_en : 1'b0;
    wire [15:0] su3_di_re = active_mode ? su2_do_re : 16'd0;
    wire [15:0] su3_di_im = active_mode ? su2_do_im : 16'd0;

    SdfUnit2 #(.WIDTH(16), .BF_RH(0)) SU3 (
        .clock  (clock      ),
        .reset  (reset      ),
        .di_en  (su3_di_en  ),
        .di_re  (su3_di_re  ),
    ...
    assign do_en = active_mode ? su3_do_en : su2_do_en;
    assign do_re = active_mode ? su3_do_re : su2_do_re;
    assign do_im = active_mode ? su3_do_im : su2_do_im;

endmodule

//----------------------------------------------------------------------
//  SdfUnitShared: Unified Radix-2^2 SDF Stage for 64- and 128-Point FFT
//----------------------------------------------------------------------
module SdfUnitShared #(
    parameter STAGE = 0,     // 0, 1, or 2
    parameter WIDTH = 16
)(
    input               clock,
    input               reset,
    input               mode,   // 0: 64-pt, 1: 128-pt
    input               di_en,
    ...
    localparam LOG_M_128 = 7 - 2 * STAGE;
    localparam LOG_M_64  = 6 - 2 * STAGE;
    localparam MAX_D1    = 1 << (LOG_M_128 - 1);
    localparam MAX_D2    = 1 << (LOG_M_128 - 2);
    localparam SHIFT     = 2 * STAGE;

    wire [6:0] d1_depth  = mode ? MAX_D1[6:0] : (MAX_D1[6:0] >> 1);
    wire [6:0] d2_depth  = mode ? MAX_D2[6:0] : (MAX_D2[6:0] >> 1);
    ...
    reg  [6:0]       di_count;
    wire             bf1_bf;
    wire [WIDTH-1:0] bf1_x0_re, bf1_x0_im;
    wire [WIDTH-1:0] bf1_x1_re, bf1_x1_im;
    ...
    wire [6:0] di_count_next = (di_count == max_count) ? 7'd0 : (di_count + 1'b1);

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            di_count <= 7'd0;
        end else begin
            di_count <= di_en ? di_count_next : 7'd0;
        end
    end

    assign bf1_bf = mode ? di_count[LOG_M_128-1] : di_count[LOG_M_64-1];

    // Clamped operand isolation: feed stable zeros when inactive
    ...
    SharedDelayBuffer #(.DEPTH_MAX(MAX_D1), .WIDTH(WIDTH)) DB1 (
        .clock  (clock    ),
        .mode   (mode     ),
        .di_re  (db1_di_re),
        .di_im  (db1_di_im),
    ...
    wire [6:0] bf1_count_next = (bf1_count == max_count) ? 7'd0 : (bf1_count + 1'b1);
    ...
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            bf1_count <= bf1_sp_en ? bf1_count_next : 7'd0;
        end
    end

    assign bf1_start = (di_count == (d1_depth - 1'b1));
    assign bf1_end   = (bf1_count == max_count);

    wire [1:0] mj_bits = mode ? bf1_count[LOG_M_128-1 : LOG_M_128-2] : bf1_count[LOG_M_64-1 : LOG_M_64-2];
    ...
    //------------------------------------------------------------------
    reg              bf2_bf;
    wire [WIDTH-1:0] bf2_x0_re, bf2_x0_im;
    wire [WIDTH-1:0] bf2_x1_re, bf2_x1_im;
    ...
    always @(posedge clock) begin
        bf2_bf <= mode ? bf1_count[LOG_M_128-2] : bf1_count[LOG_M_64-2];
    end
    ...
    SharedDelayBuffer #(.DEPTH_MAX(MAX_D2), .WIDTH(WIDTH)) DB2 (
        .clock  (clock    ),
        .mode   (mode     ),
        .di_re  (db2_di_re),
        .di_im  (db2_di_im),
    ...
    wire [6:0] bf2_count_next = (bf2_count == max_count) ? 7'd0 : (bf2_count + 1'b1);
    ...
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            bf2_count <= bf2_sp_en ? bf2_count_next : 7'd0;
        end
    end
    ...
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            bf2_start <= (bf1_count == (d2_depth - 1'b1)) & bf1_sp_en;
        end
    end

    assign bf2_end = (bf2_count == max_count);
    ...
    //------------------------------------------------------------------
    //  Multiplication and Twiddle Logic
    //------------------------------------------------------------------
    wire [1:0]       tw_sel;
    wire [4:0]       tw_num_128;
    wire [3:0]       tw_num_64;
    wire [6:0]       tw_addr_128;
    wire [5:0]       tw_addr_64;
    wire [6:0]       rom_addr;
    wire [WIDTH-1:0] tw_re, tw_im;
    reg              mu_en;
    wire [WIDTH-1:0] mu_a_re, mu_a_im;
    wire [WIDTH-1:0] mu_b_re, mu_b_im;
    wire [WIDTH-1:0] mu_m_re, mu_m_im;
    reg  [WIDTH-1:0] mu_do_re, mu_do_im;
    reg              mu_do_en;

    assign tw_sel[1] = mode ? bf2_count[LOG_M_128-2] : bf2_count[LOG_M_64-2];
    assign tw_sel[0] = mode ? bf2_count[LOG_M_128-1] : bf2_count[LOG_M_64-1];

    assign tw_num_128 = bf2_count[4:0] << SHIFT;
    assign tw_num_64  = bf2_count[3:0] << SHIFT;

    assign tw_addr_128 = tw_num_128 * tw_sel;
    assign tw_addr_64  = tw_num_64  * tw_sel;

    // Suppress twiddle table access when stage 2 is in 64-point mode
    assign rom_addr = ((STAGE == 2) && (!mode)) ? 7'd0 :
                      (mode ? tw_addr_128 : {tw_addr_64, 1'b0});
    ...
    // Operand isolation: clamp all 4 multiplier inputs to zero during zero-address bypass,
    // invalid cycles, or mode-64 stage-2 operation.
    wire mul_active = ((STAGE == 2) && (!mode)) ? 1'b0 : (mu_en && bf2_do_en);

    assign mu_a_re = mul_active ? bf2_do_re : {WIDTH{1'b0}};
    assign mu_a_im = mul_active ? bf2_do_im : {WIDTH{1'b0}};
    assign mu_b_re = mul_active ? tw_re     : {WIDTH{1'b0}};
    assign mu_b_im = mul_active ? tw_im     : {WIDTH{1'b0}};
    ...
    // Multiplication is bypassed when LOG_M == 2 (Stage 2 in 64-point mode)
    wire bypass_mult = (STAGE == 2) && (!mode);

    assign do_en = bypass_mult ? bf2_do_en : mu_do_en;
    assign do_re = bypass_mult ? bf2_do_re : mu_do_re;
    assign do_im = bypass_mult ? bf2_do_im : mu_do_im;

endmodule

//----------------------------------------------------------------------
//  SharedDelayBuffer: Dynamic Mode-Tapped Delay Line with Upper-Half Gating
//----------------------------------------------------------------------
module SharedDelayBuffer #(
    parameter DEPTH_MAX = 64,
    parameter WIDTH = 16
)(
    input                   clock,
    input                   mode,   // 0: DEPTH_MAX/2, 1: DEPTH_MAX
    input       [WIDTH-1:0] di_re,
    input       [WIDTH-1:0] di_im,
    output      [WIDTH-1:0] do_re,
    output      [WIDTH-1:0] do_im
    ...
    reg [WIDTH-1:0] buf_re[0:DEPTH_MAX-1];
    reg [WIDTH-1:0] buf_im[0:DEPTH_MAX-1];
    integer n;

    always @(posedge clock) begin
        if (mode) begin
            for (n = DEPTH_MAX-1; n > 0; n = n - 1) begin
                buf_re[n] <= buf_re[n-1];
                buf_im[n] <= buf_im[n-1];
            end
        end else begin
            for (n = (DEPTH_MAX/2)-1; n > 0; n = n - 1) begin
                buf_re[n] <= buf_re[n-1];
                buf_im[n] <= buf_im[n-1];
            end
        end
    ...
    assign do_re = mode ? buf_re[DEPTH_MAX-1] : buf_re[(DEPTH_MAX/2)-1];
    assign do_im = mode ? buf_im[DEPTH_MAX-1] : buf_im[(DEPTH_MAX/2)-1];

endmodule

//----------------------------------------------------------------------
//  DelayBuffer: Fixed Delay Buffer (Depth = 1)
//----------------------------------------------------------------------
module DelayBuffer #(
    parameter DEPTH = 1,
    parameter WIDTH = 16
...
//----------------------------------------------------------------------
//  SdfUnit2: Radix-2 SDF Dedicated for Final Stage in 128-point FFT
//----------------------------------------------------------------------
module SdfUnit2 #(
    parameter WIDTH = 16,
    parameter BF_RH = 0
    ...
    reg             bf_en;
    wire[WIDTH-1:0] x0_re, x0_im;
    wire[WIDTH-1:0] x1_re, x1_im;
...
module Twiddle #(
    ...
    wire [15:0] mx_re, mx_im;
    reg  [15:0] ff_re, ff_im;
