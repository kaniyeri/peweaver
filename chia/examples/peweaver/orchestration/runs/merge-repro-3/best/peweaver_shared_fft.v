//----------------------------------------------------------------------
//  peweaver_shared_fft: Unified 64-Point and 128-Point FFT Core
//  Radix-2^2 Single-Path Delay Feedback Architecture
//----------------------------------------------------------------------

module peweaver_shared_fft #(
    parameter WIDTH = 16
)(
    input               clock,
    input               reset,
    input               mode,   // 0 = 64-point, 1 = 128-point
    input               di_en,
    input       [15:0]  di_re,
    input       [15:0]  di_im,
    output              do_en,
    output      [15:0]  do_re,
    output      [15:0]  do_im
);

    reg busy_q;
    reg mode_q;

    wire pipe_active;
    wire active_mode = busy_q ? mode_q : mode;

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            busy_q <= 1'b0;
            mode_q <= 1'b0;
        end else begin
            if (!busy_q) begin
                if (di_en) begin
                    busy_q <= 1'b1;
                    mode_q <= mode;
                end
            end else begin
                if (!pipe_active) begin
                    busy_q <= 1'b0;
                end
            end
        end
    end

    // Stage 0: M = 64 (mode 0) / M = 128 (mode 1)
    wire        p0_do_en;
    wire [15:0] p0_do_re;
    wire [15:0] p0_do_im;
    wire        p0_active;

    peweaver_sdf_stage #(
        .MAX_M                 (128),
        .LM_MAX                (7),
        .HAS_MULT_BYPASS_MODE0 (0),
        .WIDTH                 (16)
    ) P0 (
        .clock        (clock),
        .reset        (reset),
        .mode         (active_mode),
        .di_en        (di_en),
        .di_re        (di_re),
        .di_im        (di_im),
        .do_en        (p0_do_en),
        .do_re        (p0_do_re),
        .do_im        (p0_do_im),
        .stage_active (p0_active)
    );

    // Stage 1: M = 16 (mode 0) / M = 32 (mode 1)
    wire        p1_do_en;
    wire [15:0] p1_do_re;
    wire [15:0] p1_do_im;
    wire        p1_active;

    peweaver_sdf_stage #(
        .MAX_M                 (32),
        .LM_MAX                (5),
        .HAS_MULT_BYPASS_MODE0 (0),
        .WIDTH                 (16)
    ) P1 (
        .clock        (clock),
        .reset        (reset),
        .mode         (active_mode),
        .di_en        (p0_do_en),
        .di_re        (p0_do_re),
        .di_im        (p0_do_im),
        .do_en        (p1_do_en),
        .do_re        (p1_do_re),
        .do_im        (p1_do_im),
        .stage_active (p1_active)
    );

    // Stage 2: M = 4 (mode 0, mult bypass) / M = 8 (mode 1)
    wire        p2_do_en;
    wire [15:0] p2_do_re;
    wire [15:0] p2_do_im;
    wire        p2_active;

    peweaver_sdf_stage #(
        .MAX_M                 (8),
        .LM_MAX                (3),
        .HAS_MULT_BYPASS_MODE0 (1),
        .WIDTH                 (16)
    ) P2 (
        .clock        (clock),
        .reset        (reset),
        .mode         (active_mode),
        .di_en        (p1_do_en),
        .di_re        (p1_do_re),
        .di_im        (p1_do_im),
        .do_en        (p2_do_en),
        .do_re        (p2_do_re),
        .do_im        (p2_do_im),
        .stage_active (p2_active)
    );

    // Stage 3: Radix-2 SdfUnit2 (Active only in 128-point mode)
    wire        p3_di_en = active_mode ? p2_do_en : 1'b0;
    wire [15:0] p3_di_re = p2_do_re;
    wire [15:0] p3_di_im = p2_do_im;

    wire        p3_do_en;
    wire [15:0] p3_do_re;
    wire [15:0] p3_do_im;
    wire        p3_active;

    peweaver_sdf_unit2 #(.WIDTH(16), .BF_RH(0)) P3 (
        .clock        (clock),
        .reset        (reset),
        .di_en        (p3_di_en),
        .di_re        (p3_di_re),
        .di_im        (p3_di_im),
        .do_en        (p3_do_en),
        .do_re        (p3_do_re),
        .do_im        (p3_do_im),
        .stage_active (p3_active)
    );

    // Output Selection: 64-point outputs directly from P2; 128-point from P3
    assign do_en = active_mode ? p3_do_en : p2_do_en;
    assign do_re = active_mode ? p3_do_re : p2_do_re;
    assign do_im = active_mode ? p3_do_im : p2_do_im;

    assign pipe_active = di_en | p0_active | p1_active | p2_active | p3_active | do_en;

endmodule

//----------------------------------------------------------------------
//  peweaver_sdf_stage: Shared Configurable Radix-2^2 SDF Stage
//----------------------------------------------------------------------
module peweaver_sdf_stage #(
    parameter MAX_M                 = 128,
    parameter LM_MAX                = 7,
    parameter HAS_MULT_BYPASS_MODE0 = 0,
    parameter WIDTH                 = 16
)(
    input               clock,
    input               reset,
    input               mode,
    input               di_en,
    input   [WIDTH-1:0] di_re,
    input   [WIDTH-1:0] di_im,
    output              do_en,
    output  [WIDTH-1:0] do_re,
    output  [WIDTH-1:0] do_im,
    output              stage_active
);

    wire [6:0] count_max = mode ? 7'd127 : 7'd63;

    // 1st Butterfly
    reg [6:0] di_count;
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            di_count <= 7'd0;
        end else begin
            di_count <= di_en ? ((di_count == count_max) ? 7'd0 : (di_count + 1'b1)) : 7'd0;
        end
    end

    wire bf1_bf = mode ? di_count[LM_MAX-1] : di_count[LM_MAX-2];

    wire [WIDTH-1:0] db1_do_re, db1_do_im;
    wire [WIDTH-1:0] bf1_x0_re = db1_do_re;
    wire [WIDTH-1:0] bf1_x0_im = db1_do_im;
    wire [WIDTH-1:0] bf1_x1_re = di_re;
    wire [WIDTH-1:0] bf1_x1_im = di_im;
    wire [WIDTH-1:0] bf1_y0_re, bf1_y0_im, bf1_y1_re, bf1_y1_im;

    peweaver_butterfly #(.WIDTH(WIDTH), .RH(0)) BF1 (
        .x0_re (bf1_x0_re),
        .x0_im (bf1_x0_im),
        .x1_re (bf1_x1_re),
        .x1_im (bf1_x1_im),
        .y0_re (bf1_y0_re),
        .y0_im (bf1_y0_im),
        .y1_re (bf1_y1_re),
        .y1_im (bf1_y1_im)
    );

    wire [WIDTH-1:0] db1_di_re = bf1_bf ? bf1_y1_re : di_re;
    wire [WIDTH-1:0] db1_di_im = bf1_bf ? bf1_y1_im : di_im;

    peweaver_delay_buffer #(.MAX_DEPTH(MAX_M/2), .WIDTH(WIDTH)) DB1 (
        .clock (clock),
        .mode  (mode),
        .di_re (db1_di_re),
        .di_im (db1_di_im),
        .do_re (db1_do_re),
        .do_im (db1_do_im)
    );

    wire [6:0] bf1_start_count = mode ? (MAX_M/2 - 1) : (MAX_M/4 - 1);
    wire bf1_start = (di_count == bf1_start_count);

    reg       bf1_sp_en;
    reg [6:0] bf1_count;
    wire      bf1_end = (bf1_count == count_max);

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            bf1_sp_en <= 1'b0;
            bf1_count <= 7'd0;
        end else begin
            bf1_sp_en <= bf1_start ? 1'b1 : bf1_end ? 1'b0 : bf1_sp_en;
            bf1_count <= bf1_sp_en ? ((bf1_count == count_max) ? 7'd0 : (bf1_count + 1'b1)) : 7'd0;
        end
    end

    wire bf1_mj = mode ? (bf1_count[LM_MAX-1:LM_MAX-2] == 2'd3)
                       : (bf1_count[LM_MAX-2:LM_MAX-3] == 2'd3);

    wire [WIDTH-1:0] bf1_sp_re = bf1_bf ? bf1_y0_re : (bf1_mj ?  db1_do_im : db1_do_re);
    wire [WIDTH-1:0] bf1_sp_im = bf1_bf ? bf1_y0_im : (bf1_mj ? -db1_do_re : db1_do_im);

    reg [WIDTH-1:0] bf1_do_re;
    reg [WIDTH-1:0] bf1_do_im;

    always @(posedge clock) begin
        bf1_do_re <= bf1_sp_re;
        bf1_do_im <= bf1_sp_im;
    end

    // 2nd Butterfly
    reg bf2_bf;
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            bf2_bf <= 1'b0;
        end else begin
            bf2_bf <= mode ? bf1_count[LM_MAX-2] : bf1_count[LM_MAX-3];
        end
    end

    reg bf2_start;
    wire [6:0] bf2_start_count = mode ? (MAX_M/4 - 1) : (MAX_M/8 - 1);

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            bf2_start <= 1'b0;
        end else begin
            bf2_start <= (bf1_count == bf2_start_count) & bf1_sp_en;
        end
    end

    reg       bf2_sp_en;
    reg [6:0] bf2_count;
    wire      bf2_end = (bf2_count == count_max);

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            bf2_sp_en <= 1'b0;
            bf2_count <= 7'd0;
        end else begin
            bf2_sp_en <= bf2_start ? 1'b1 : bf2_end ? 1'b0 : bf2_sp_en;
            bf2_count <= bf2_sp_en ? ((bf2_count == count_max) ? 7'd0 : (bf2_count + 1'b1)) : 7'd0;
        end
    end

    wire [WIDTH-1:0] db2_do_re, db2_do_im;
    wire [WIDTH-1:0] bf2_x0_re = db2_do_re;
    wire [WIDTH-1:0] bf2_x0_im = db2_do_im;
    wire [WIDTH-1:0] bf2_x1_re = bf1_do_re;
    wire [WIDTH-1:0] bf2_x1_im = bf1_do_im;
    wire [WIDTH-1:0] bf2_y0_re, bf2_y0_im, bf2_y1_re, bf2_y1_im;

    peweaver_butterfly #(.WIDTH(WIDTH), .RH(1)) BF2 (
        .x0_re (bf2_x0_re),
        .x0_im (bf2_x0_im),
        .x1_re (bf2_x1_re),
        .x1_im (bf2_x1_im),
        .y0_re (bf2_y0_re),
        .y0_im (bf2_y0_im),
        .y1_re (bf2_y1_re),
        .y1_im (bf2_y1_im)
    );

    wire [WIDTH-1:0] db2_di_re = bf2_bf ? bf2_y1_re : bf1_do_re;
    wire [WIDTH-1:0] db2_di_im = bf2_bf ? bf2_y1_im : bf1_do_im;

    peweaver_delay_buffer #(.MAX_DEPTH(MAX_M/4), .WIDTH(WIDTH)) DB2 (
        .clock (clock),
        .mode  (mode),
        .di_re (db2_di_re),
        .di_im (db2_di_im),
        .do_re (db2_do_re),
        .do_im (db2_do_im)
    );

    wire [WIDTH-1:0] bf2_sp_re = bf2_bf ? bf2_y0_re : db2_do_re;
    wire [WIDTH-1:0] bf2_sp_im = bf2_bf ? bf2_y0_im : db2_do_im;

    reg [WIDTH-1:0] bf2_do_re;
    reg [WIDTH-1:0] bf2_do_im;
    reg             bf2_do_en;

    always @(posedge clock) begin
        bf2_do_re <= bf2_sp_re;
        bf2_do_im <= bf2_sp_im;
    end

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            bf2_do_en <= 1'b0;
        end else begin
            bf2_do_en <= bf2_sp_en;
        end
    end

    // Multiplication & Twiddle Lookup
    wire [1:0] tw_sel = mode ? {bf2_count[LM_MAX-2], bf2_count[LM_MAX-1]}
                             : {bf2_count[LM_MAX-3], bf2_count[LM_MAX-2]};

    wire [4:0] tw_num128 = (bf2_count << (7 - LM_MAX));
    wire [3:0] tw_num64  = (bf2_count << (7 - LM_MAX));
    wire [6:0] tw_addr128 = tw_num128 * tw_sel;
    wire [5:0] tw_addr64  = tw_num64 * tw_sel;
    wire [6:0] tw_addr    = mode ? tw_addr128 : {tw_addr64, 1'b0};

    wire [WIDTH-1:0] tw_re, tw_im;

    peweaver_twiddle TW (
        .clock (clock),
        .addr  (tw_addr),
        .tw_re (tw_re),
        .tw_im (tw_im)
    );

    reg mu_en;
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            mu_en <= 1'b0;
        end else begin
            mu_en <= (tw_addr != 7'd0);
        end
    end

    wire mult_disable = HAS_MULT_BYPASS_MODE0 && !mode;

    wire [WIDTH-1:0] mu_m_re, mu_m_im;

    peweaver_multiply #(.WIDTH(WIDTH)) MU (
        .a_re (bf2_do_re),
        .a_im (bf2_do_im),
        .b_re (tw_re),
        .b_im (tw_im),
        .m_re (mu_m_re),
        .m_im (mu_m_im)
    );

    reg [WIDTH-1:0] mu_do_re;
    reg [WIDTH-1:0] mu_do_im;
    reg             mu_do_en;

    always @(posedge clock) begin
        mu_do_re <= mu_en ? mu_m_re : bf2_do_re;
        mu_do_im <= mu_en ? mu_m_im : bf2_do_im;
    end

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            mu_do_en <= 1'b0;
        end else begin
            mu_do_en <= bf2_do_en;
        end
    end

    assign do_en = mult_disable ? bf2_do_en : mu_do_en;
    assign do_re = mult_disable ? bf2_do_re : mu_do_re;
    assign do_im = mult_disable ? bf2_do_im : mu_do_im;

    assign stage_active = di_en | bf1_sp_en | bf2_sp_en | bf2_do_en | mu_do_en;

endmodule

//----------------------------------------------------------------------
//  peweaver_sdf_unit2: Radix-2 SDF Stage Dedicated for M = 2
//----------------------------------------------------------------------
module peweaver_sdf_unit2 #(
    parameter   WIDTH = 16,
    parameter   BF_RH = 0
)(
    input                   clock,
    input                   reset,
    input                   di_en,
    input       [WIDTH-1:0] di_re,
    input       [WIDTH-1:0] di_im,
    output  reg             do_en,
    output  reg [WIDTH-1:0] do_re,
    output  reg [WIDTH-1:0] do_im,
    output                  stage_active
);

    reg             bf_en;
    wire[WIDTH-1:0] x0_re = db_do_re;
    wire[WIDTH-1:0] x0_im = db_do_im;
    wire[WIDTH-1:0] x1_re = di_re;
    wire[WIDTH-1:0] x1_im = di_im;
    wire[WIDTH-1:0] y0_re;
    wire[WIDTH-1:0] y0_im;
    wire[WIDTH-1:0] y1_re;
    wire[WIDTH-1:0] y1_im;
    wire[WIDTH-1:0] db_di_re;
    wire[WIDTH-1:0] db_di_im;
    wire[WIDTH-1:0] db_do_re;
    wire[WIDTH-1:0] db_do_im;
    wire[WIDTH-1:0] bf_sp_re;
    wire[WIDTH-1:0] bf_sp_im;
    reg             bf_sp_en;

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            bf_en <= 1'b0;
        end else begin
            bf_en <= di_en ? ~bf_en : 1'b0;
        end
    end

    peweaver_butterfly #(.WIDTH(WIDTH), .RH(BF_RH)) BF (
        .x0_re (x0_re),
        .x0_im (x0_im),
        .x1_re (x1_re),
        .x1_im (x1_im),
        .y0_re (y0_re),
        .y0_im (y0_im),
        .y1_re (y1_re),
        .y1_im (y1_im)
    );

    peweaver_delay_buffer1 #(.WIDTH(WIDTH)) DB (
        .clock (clock),
        .di_re (db_di_re),
        .di_im (db_di_im),
        .do_re (db_do_re),
        .do_im (db_do_im)
    );

    assign db_di_re = bf_en ? y1_re : di_re;
    assign db_di_im = bf_en ? y1_im : di_im;
    assign bf_sp_re = bf_en ? y0_re : db_do_re;
    assign bf_sp_im = bf_en ? y0_im : db_do_im;

    always @(posedge clock or posedge reset) begin
        if (reset) begin
            bf_sp_en <= 1'b0;
            do_en    <= 1'b0;
        end else begin
            bf_sp_en <= di_en;
            do_en    <= bf_sp_en;
        end
    end

    always @(posedge clock) begin
        do_re <= bf_sp_re;
        do_im <= bf_sp_im;
    end

    assign stage_active = di_en | bf_sp_en | do_en;

endmodule

//----------------------------------------------------------------------
//  peweaver_butterfly: Complex Add/Sub and Arithmetic Right Shift
//----------------------------------------------------------------------
module peweaver_butterfly #(
    parameter   WIDTH = 16,
    parameter   RH = 0
)(
    input   signed  [WIDTH-1:0] x0_re,
    input   signed  [WIDTH-1:0] x0_im,
    input   signed  [WIDTH-1:0] x1_re,
    input   signed  [WIDTH-1:0] x1_im,
    output  signed  [WIDTH-1:0] y0_re,
    output  signed  [WIDTH-1:0] y0_im,
    output  signed  [WIDTH-1:0] y1_re,
    output  signed  [WIDTH-1:0] y1_im
);

    wire signed [WIDTH:0] add_re = x0_re + x1_re;
    wire signed [WIDTH:0] add_im = x0_im + x1_im;
    wire signed [WIDTH:0] sub_re = x0_re - x1_re;
    wire signed [WIDTH:0] sub_im = x0_im - x1_im;

    assign y0_re = (add_re + RH) >>> 1;
    assign y0_im = (add_im + RH) >>> 1;
    assign y1_re = (sub_re + RH) >>> 1;
    assign y1_im = (sub_im + RH) >>> 1;

endmodule

//----------------------------------------------------------------------
//  peweaver_multiply: Exact Signed 16x16 Complex Multiplier
//----------------------------------------------------------------------
module peweaver_multiply #(
    parameter   WIDTH = 16
)(
    input   signed  [WIDTH-1:0] a_re,
    input   signed  [WIDTH-1:0] a_im,
    input   signed  [WIDTH-1:0] b_re,
    input   signed  [WIDTH-1:0] b_im,
    output  signed  [WIDTH-1:0] m_re,
    output  signed  [WIDTH-1:0] m_im
);

    wire signed [31:0] rr_full = $signed(a_re) * $signed(b_re);
    wire signed [31:0] ii_full = $signed(a_im) * $signed(b_im);
    wire signed [31:0] ri_full = $signed(a_re) * $signed(b_im);
    wire signed [31:0] ir_full = $signed(a_im) * $signed(b_re);

    wire signed [15:0] rr_q15 = rr_full[30:15];
    wire signed [15:0] ii_q15 = ii_full[30:15];
    wire signed [15:0] ri_q15 = ri_full[30:15];
    wire signed [15:0] ir_q15 = ir_full[30:15];

    assign m_re = rr_q15 - ii_q15;
    assign m_im = ri_q15 + ir_q15;

endmodule

//----------------------------------------------------------------------
//  peweaver_delay_buffer: Configurable Dual-Depth Delay Buffer
//----------------------------------------------------------------------
module peweaver_delay_buffer #(
    parameter   MAX_DEPTH = 64,
    parameter   WIDTH = 16
)(
    input               clock,
    input               mode,
    input   [WIDTH-1:0] di_re,
    input   [WIDTH-1:0] di_im,
    output  [WIDTH-1:0] do_re,
    output  [WIDTH-1:0] do_im
);

    reg [WIDTH-1:0] buf_re[0:MAX_DEPTH-1];
    reg [WIDTH-1:0] buf_im[0:MAX_DEPTH-1];
    integer n;

    always @(posedge clock) begin
        for (n = MAX_DEPTH-1; n > 0; n = n - 1) begin
            buf_re[n] <= buf_re[n-1];
            buf_im[n] <= buf_im[n-1];
        end
        buf_re[0] <= di_re;
        buf_im[0] <= di_im;
    end

    assign do_re = mode ? buf_re[MAX_DEPTH-1] : buf_re[MAX_DEPTH/2-1];
    assign do_im = mode ? buf_im[MAX_DEPTH-1] : buf_im[MAX_DEPTH/2-1];

endmodule

//----------------------------------------------------------------------
//  peweaver_delay_buffer1: 1-Cycle Feedback Delay Register
//----------------------------------------------------------------------
module peweaver_delay_buffer1 #(
    parameter   WIDTH = 16
)(
    input               clock,
    input   [WIDTH-1:0] di_re,
    input   [WIDTH-1:0] di_im,
    output  [WIDTH-1:0] do_re,
    output  [WIDTH-1:0] do_im
);

    reg [WIDTH-1:0] buf_re;
    reg [WIDTH-1:0] buf_im;

    always @(posedge clock) begin
        buf_re <= di_re;
        buf_im <= di_im;
    end

    assign do_re = buf_re;
    assign do_im = buf_im;

endmodule

//----------------------------------------------------------------------
//  peweaver_twiddle: Canonical 128-Point Twiddle Coefficient Table
//----------------------------------------------------------------------
module peweaver_twiddle #(
    parameter   TW_FF = 1
)(
    input           clock,
    input   [6:0]   addr,
    output  [15:0]  tw_re,
    output  [15:0]  tw_im
);

    wire [15:0] wn_re[0:127];
    wire [15:0] wn_im[0:127];
    wire [15:0] mx_re;
    wire [15:0] mx_im;
    reg  [15:0] ff_re;
    reg  [15:0] ff_im;

    assign mx_re = wn_re[addr];
    assign mx_im = wn_im[addr];

    always @(posedge clock) begin
        ff_re <= mx_re;
        ff_im <= mx_im;
    end

    assign tw_re = TW_FF ? ff_re : mx_re;
    assign tw_im = TW_FF ? ff_im : mx_im;

    assign wn_re[  0] = 16'h0000; assign wn_im[  0] = 16'h0000;
    assign wn_re[  1] = 16'h7FD9; assign wn_im[  1] = 16'hF9B8;
    assign wn_re[  2] = 16'h7F62; assign wn_im[  2] = 16'hF374;
    assign wn_re[  3] = 16'h7E9D; assign wn_im[  3] = 16'hED38;
    assign wn_re[  4] = 16'h7D8A; assign wn_im[  4] = 16'hE707;
    assign wn_re[  5] = 16'h7C2A; assign wn_im[  5] = 16'hE0E6;
    assign wn_re[  6] = 16'h7A7D; assign wn_im[  6] = 16'hDAD8;
    assign wn_re[  7] = 16'h7885; assign wn_im[  7] = 16'hD4E1;
    assign wn_re[  8] = 16'h7642; assign wn_im[  8] = 16'hCF04;
    assign wn_re[  9] = 16'h73B6; assign wn_im[  9] = 16'hC946;
    assign wn_re[ 10] = 16'h70E3; assign wn_im[ 10] = 16'hC3A9;
    assign wn_re[ 11] = 16'h6DCA; assign wn_im[ 11] = 16'hBE32;
    assign wn_re[ 12] = 16'h6A6E; assign wn_im[ 12] = 16'hB8E3;
    assign wn_re[ 13] = 16'h66D0; assign wn_im[ 13] = 16'hB3C0;
    assign wn_re[ 14] = 16'h62F2; assign wn_im[ 14] = 16'hAECC;
    assign wn_re[ 15] = 16'h5ED7; assign wn_im[ 15] = 16'hAA0A;
    assign wn_re[ 16] = 16'h5A82; assign wn_im[ 16] = 16'hA57E;
    assign wn_re[ 17] = 16'h55F6; assign wn_im[ 17] = 16'hA129;
    assign wn_re[ 18] = 16'h5134; assign wn_im[ 18] = 16'h9D0E;
    assign wn_re[ 19] = 16'h4C40; assign wn_im[ 19] = 16'h9930;
    assign wn_re[ 20] = 16'h471D; assign wn_im[ 20] = 16'h9592;
    assign wn_re[ 21] = 16'h41CE; assign wn_im[ 21] = 16'h9236;
    assign wn_re[ 22] = 16'h3C57; assign wn_im[ 22] = 16'h8F1D;
    assign wn_re[ 23] = 16'h36BA; assign wn_im[ 23] = 16'h8C4A;
    assign wn_re[ 24] = 16'h30FC; assign wn_im[ 24] = 16'h89BE;
    assign wn_re[ 25] = 16'h2B1F; assign wn_im[ 25] = 16'h877B;
    assign wn_re[ 26] = 16'h2528; assign wn_im[ 26] = 16'h8583;
    assign wn_re[ 27] = 16'h1F1A; assign wn_im[ 27] = 16'h83D6;
    assign wn_re[ 28] = 16'h18F9; assign wn_im[ 28] = 16'h8276;
    assign wn_re[ 29] = 16'h12C8; assign wn_im[ 29] = 16'h8163;
    assign wn_re[ 30] = 16'h0C8C; assign wn_im[ 30] = 16'h809E;
    assign wn_re[ 31] = 16'h0648; assign wn_im[ 31] = 16'h8027;
    assign wn_re[ 32] = 16'h0000; assign wn_im[ 32] = 16'h8000;
    assign wn_re[ 33] = 16'hF9B8; assign wn_im[ 33] = 16'h8027;
    assign wn_re[ 34] = 16'hF374; assign wn_im[ 34] = 16'h809E;
    assign wn_re[ 35] = 16'hxxxx; assign wn_im[ 35] = 16'hxxxx;
    assign wn_re[ 36] = 16'hE707; assign wn_im[ 36] = 16'h8276;
    assign wn_re[ 37] = 16'hxxxx; assign wn_im[ 37] = 16'hxxxx;
    assign wn_re[ 38] = 16'hDAD8; assign wn_im[ 38] = 16'h8583;
    assign wn_re[ 39] = 16'hD4E1; assign wn_im[ 39] = 16'h877B;
    assign wn_re[ 40] = 16'hCF04; assign wn_im[ 40] = 16'h89BE;
    assign wn_re[ 41] = 16'hxxxx; assign wn_im[ 41] = 16'hxxxx;
    assign wn_re[ 42] = 16'hC3A9; assign wn_im[ 42] = 16'h8F1D;
    assign wn_re[ 43] = 16'hxxxx; assign wn_im[ 43] = 16'hxxxx;
    assign wn_re[ 44] = 16'hB8E3; assign wn_im[ 44] = 16'h9592;
    assign wn_re[ 45] = 16'hB3C0; assign wn_im[ 45] = 16'h9930;
    assign wn_re[ 46] = 16'hAECC; assign wn_im[ 46] = 16'h9D0E;
    assign wn_re[ 47] = 16'hxxxx; assign wn_im[ 47] = 16'hxxxx;
    assign wn_re[ 48] = 16'hA57E; assign wn_im[ 48] = 16'hA57E;
    assign wn_re[ 49] = 16'hxxxx; assign wn_im[ 49] = 16'hxxxx;
    assign wn_re[ 50] = 16'h9D0E; assign wn_im[ 50] = 16'hAECC;
    assign wn_re[ 51] = 16'h9930; assign wn_im[ 51] = 16'hB3C0;
    assign wn_re[ 52] = 16'h9592; assign wn_im[ 52] = 16'hB8E3;
    assign wn_re[ 53] = 16'hxxxx; assign wn_im[ 53] = 16'hxxxx;
    assign wn_re[ 54] = 16'h8F1D; assign wn_im[ 54] = 16'hC3A9;
    assign wn_re[ 55] = 16'hxxxx; assign wn_im[ 55] = 16'hxxxx;
    assign wn_re[ 56] = 16'h89BE; assign wn_im[ 56] = 16'hCF04;
    assign wn_re[ 57] = 16'h877B; assign wn_im[ 57] = 16'hD4E1;
    assign wn_re[ 58] = 16'h8583; assign wn_im[ 58] = 16'hDAD8;
    assign wn_re[ 59] = 16'hxxxx; assign wn_im[ 59] = 16'hxxxx;
    assign wn_re[ 60] = 16'h8276; assign wn_im[ 60] = 16'hE707;
    assign wn_re[ 61] = 16'hxxxx; assign wn_im[ 61] = 16'hxxxx;
    assign wn_re[ 62] = 16'h809E; assign wn_im[ 62] = 16'hF374;
    assign wn_re[ 63] = 16'h8027; assign wn_im[ 63] = 16'hF9B8;
    assign wn_re[ 64] = 16'hxxxx; assign wn_im[ 64] = 16'hxxxx;
    assign wn_re[ 65] = 16'hxxxx; assign wn_im[ 65] = 16'hxxxx;
    assign wn_re[ 66] = 16'h809E; assign wn_im[ 66] = 16'h0C8C;
    assign wn_re[ 67] = 16'hxxxx; assign wn_im[ 67] = 16'hxxxx;
    assign wn_re[ 68] = 16'hxxxx; assign wn_im[ 68] = 16'hxxxx;
    assign wn_re[ 69] = 16'h83D6; assign wn_im[ 69] = 16'h1F1A;
    assign wn_re[ 70] = 16'hxxxx; assign wn_im[ 70] = 16'hxxxx;
    assign wn_re[ 71] = 16'hxxxx; assign wn_im[ 71] = 16'hxxxx;
    assign wn_re[ 72] = 16'h89BE; assign wn_im[ 72] = 16'h30FC;
    assign wn_re[ 73] = 16'hxxxx; assign wn_im[ 73] = 16'hxxxx;
    assign wn_re[ 74] = 16'hxxxx; assign wn_im[ 74] = 16'hxxxx;
    assign wn_re[ 75] = 16'h9236; assign wn_im[ 75] = 16'h41CE;
    assign wn_re[ 76] = 16'hxxxx; assign wn_im[ 76] = 16'hxxxx;
    assign wn_re[ 77] = 16'hxxxx; assign wn_im[ 77] = 16'hxxxx;
    assign wn_re[ 78] = 16'h9D0E; assign wn_im[ 78] = 16'h5134;
    assign wn_re[ 79] = 16'hxxxx; assign wn_im[ 79] = 16'hxxxx;
    assign wn_re[ 80] = 16'hxxxx; assign wn_im[ 80] = 16'hxxxx;
    assign wn_re[ 81] = 16'hAA0A; assign wn_im[ 81] = 16'h5ED7;
    assign wn_re[ 82] = 16'hxxxx; assign wn_im[ 82] = 16'hxxxx;
    assign wn_re[ 83] = 16'hxxxx; assign wn_im[ 83] = 16'hxxxx;
    assign wn_re[ 84] = 16'hB8E3; assign wn_im[ 84] = 16'h6A6E;
    assign wn_re[ 85] = 16'hxxxx; assign wn_im[ 85] = 16'hxxxx;
    assign wn_re[ 86] = 16'hxxxx; assign wn_im[ 86] = 16'hxxxx;
    assign wn_re[ 87] = 16'hC946; assign wn_im[ 87] = 16'h73B6;
    assign wn_re[ 88] = 16'hxxxx; assign wn_im[ 88] = 16'hxxxx;
    assign wn_re[ 89] = 16'hxxxx; assign wn_im[ 89] = 16'hxxxx;
    assign wn_re[ 90] = 16'hDAD8; assign wn_im[ 90] = 16'h7A7D;
    assign wn_re[ 91] = 16'hxxxx; assign wn_im[ 91] = 16'hxxxx;
    assign wn_re[ 92] = 16'hxxxx; assign wn_im[ 92] = 16'hxxxx;
    assign wn_re[ 93] = 16'hED38; assign wn_im[ 93] = 16'h7E9D;
    assign wn_re[ 94] = 16'hxxxx; assign wn_im[ 94] = 16'hxxxx;
    assign wn_re[ 95] = 16'hxxxx; assign wn_im[ 95] = 16'hxxxx;
    assign wn_re[ 96] = 16'hxxxx; assign wn_im[ 96] = 16'hxxxx;
    assign wn_re[ 97] = 16'hxxxx; assign wn_im[ 97] = 16'hxxxx;
    assign wn_re[ 98] = 16'hxxxx; assign wn_im[ 98] = 16'hxxxx;
    assign wn_re[ 99] = 16'hxxxx; assign wn_im[ 99] = 16'hxxxx;
    assign wn_re[100] = 16'hxxxx; assign wn_im[100] = 16'hxxxx;
    assign wn_re[101] = 16'hxxxx; assign wn_im[101] = 16'hxxxx;
    assign wn_re[102] = 16'hxxxx; assign wn_im[102] = 16'hxxxx;
    assign wn_re[103] = 16'hxxxx; assign wn_im[103] = 16'hxxxx;
    assign wn_re[104] = 16'hxxxx; assign wn_im[104] = 16'hxxxx;
    assign wn_re[105] = 16'hxxxx; assign wn_im[105] = 16'hxxxx;
    assign wn_re[106] = 16'hxxxx; assign wn_im[106] = 16'hxxxx;
    assign wn_re[107] = 16'hxxxx; assign wn_im[107] = 16'hxxxx;
    assign wn_re[108] = 16'hxxxx; assign wn_im[108] = 16'hxxxx;
    assign wn_re[109] = 16'hxxxx; assign wn_im[109] = 16'hxxxx;
    assign wn_re[110] = 16'hxxxx; assign wn_im[110] = 16'hxxxx;
    assign wn_re[111] = 16'hxxxx; assign wn_im[111] = 16'hxxxx;
    assign wn_re[112] = 16'hxxxx; assign wn_im[112] = 16'hxxxx;
    assign wn_re[113] = 16'hxxxx; assign wn_im[113] = 16'hxxxx;
    assign wn_re[114] = 16'hxxxx; assign wn_im[114] = 16'hxxxx;
    assign wn_re[115] = 16'hxxxx; assign wn_im[115] = 16'hxxxx;
    assign wn_re[116] = 16'hxxxx; assign wn_im[116] = 16'hxxxx;
    assign wn_re[117] = 16'hxxxx; assign wn_im[117] = 16'hxxxx;
    assign wn_re[118] = 16'hxxxx; assign wn_im[118] = 16'hxxxx;
    assign wn_re[119] = 16'hxxxx; assign wn_im[119] = 16'hxxxx;
    assign wn_re[120] = 16'hxxxx; assign wn_im[120] = 16'hxxxx;
    assign wn_re[121] = 16'hxxxx; assign wn_im[121] = 16'hxxxx;
    assign wn_re[122] = 16'hxxxx; assign wn_im[122] = 16'hxxxx;
    assign wn_re[123] = 16'hxxxx; assign wn_im[123] = 16'hxxxx;
    assign wn_re[124] = 16'hxxxx; assign wn_im[124] = 16'hxxxx;
    assign wn_re[125] = 16'hxxxx; assign wn_im[125] = 16'hxxxx;
    assign wn_re[126] = 16'hxxxx; assign wn_im[126] = 16'hxxxx;
    assign wn_re[127] = 16'hxxxx; assign wn_im[127] = 16'hxxxx;

endmodule
