//----------------------------------------------------------------------
// PEWeaver directed regression testbench for the curated r22sdf 128-point
// FFT reference (HALO-inspired stateful FFT proxy, Phase A2).
//
// Interface/control validation of the stateful reference:
//   1. asynchronous active-high reset (asserted away from clock edges);
//   2. exactly 128 consecutive valid input samples per frame;
//   3. no output valid before the documented pipeline latency;
//   4. exactly 128 consecutive output-valid samples per frame;
...
//
// Vectors: upstream r22sdf sim/fft_128 input4/input5/output4/output5
...
// sample bitrev7(n) because the reference emits bit-reversed order.
//
// Two-state policy: upstream Twiddle64.v/Twiddle128.v and SdfUnit*.v contain
// deliberate 1'bx assignments ("Set unknown value x for verification") for
// paths that are never selected. Verilator resolves x to 0 (--x-assign 0
// --x-initial 0); the testbench also drives defined zero on idle inputs so
// delay-line contents are deterministic. This regression validates
// interface/control behavior and reproduces the upstream goldens; it is NOT
// an independent numerical FFT oracle.
//----------------------------------------------------------------------
`timescale 1ns/1ns

module peweaver_fft128_directed_tb;

    // Documented pipeline latency of FFT128 (upstream header comment: 137
    ...
    localparam int FIRST_OUT = 137;

    localparam int FRAME = 128;
    localparam int DRAIN = 256;  // > deepest DelayBuffer depth (64, SU1 with M=128) plus margin
    ...
    peweaver_halo_fft128_reference dut (
        .clock (clock),
        .reset (reset),
        .di_en (di_en),
        .di_re (di_re),
    ...
    function automatic int bitrev7(input int x);
        int r = 0;
        for (int b = 0; b < 7; b++)
            if (((x >> b) & 1) != 0) r = r | (1 << (6 - b));
        return r;
    endfunction
        ...
        begin
            ...
            if (n_out == FRAME) begin
                for (int n = 0; n < FRAME; n++) begin
                    if (got_re[bitrev7(n)] !== gmem[2*n] || got_im[bitrev7(n)] !== gmem[2*n + 1]) begin
                        mismatches++;
                        if (mismatches <= 4)
                            $display("[FAIL] %s: bin %0d got (%h,%h) expected (%h,%h)",
                                     label, n, got_re[bitrev7(n)], got_im[bitrev7(n)], gmem[2*n], gmem[2*n + 1]);
                    end
                end
            end
            check(mismatches == 0, $sformatf("%s: %0d/%0d output samples mismatch golden", label, mismatches, FRAME));
            if (errors == 0) $display("[ ok ] %s: latency=%0d, 128 consecutive outputs match golden", label, first_out);
        end
    endtask
    ...
    initial begin : stimulus
        ...
        if (errors != 0) begin
            $display("PEWEAVER_SIM_FAIL halo_fft128_reference errors=%0d", errors);
        end else begin
            $display("PEWEAVER_SIM_PASS halo_fft128_reference");
        end
        $finish;
    end
    ...
    initial begin : watchdog
        ...
        $display("PEWEAVER_SIM_FAIL halo_fft128_reference watchdog timeout");
        $finish;
    end
