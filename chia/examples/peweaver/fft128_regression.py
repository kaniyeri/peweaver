"""Fail-closed directed Verilator regression for the FFT128 reference fixture.

Phase A2 stateful reference integration: compiles the curated r22sdf 128-point
...
DEFAULT_FIXTURE = Path(__file__).resolve().parent / "benchmarks" / "halo_fft128_reference"
PASS_MARKER = "PEWEAVER_SIM_PASS halo_fft128_reference"
...
def resolve_verilator() -> str | None:
    ...
    if configured:
        return configured
    # <repo>/examples/peweaver/fft128_regression.py -> <repo>/../toolchains
...
def load_vector_file(path: Path) -> list[tuple[int, int]]:
    """Parse an upstream r22sdf hex vector file into 128 (re, im) sample pairs."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.fixture.is_dir():
        print(f"fft128_regression.py: fixture directory not found: {args.fixture}", file=sys.stderr)
        return 2
