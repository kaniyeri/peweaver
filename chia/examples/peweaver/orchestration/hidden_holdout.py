...
def apply_mutation(candidate_text: str, mutation: str) -> str:
    if mutation == "twiddle_re5":
        ...
        if not m:
            # generic form: any signed Q1.15 twiddle literal table entry whose
            # regex index matches address 5 in whatever table style exists
            m = re.search(r"5\s*:\s*\w+\s*=\s*16'sh([0-9A-Fa-f]{4})", candidate_text)
        if not m:
    ...
    if mutation == "scale_shift":
        if "arbr >>> (WIDTH-1)" in candidate_text:
            return candidate_text.replace("arbr >>> (WIDTH-1)",
                                          "arbr >>> (WIDTH-2)", 1)
        m = re.search(r">>>\s*15", candidate_text)
        if not m:
            raise RuntimeError("scale_shift target not found")
        return (candidate_text[:m.start()] + ">>> 14" + candidate_text[m.end():])
    ...
    if mutation == "literal_lsb":
        for m in re.finditer(r"16'h([0-9A-Fa-f]{4})", candidate_text):
            v = int(m.group(1), 16)
            if v not in (0x0000, 0xFFFF) and (v >> 4) not in (0, 0xFFFF // 16):
                v ^= 1
                return candidate_text[:m.start(1)] + f"{v:04X}" + candidate_text[m.end(1):]
        raise RuntimeError("literal_lsb target not found")
...
def main() -> int:
    ...
    if args.sensitivity:
        ...
        for mut in ("twiddle_re5", "twiddle_im5", "scale_shift",
                    "counter_wrap", "literal_lsb"):
