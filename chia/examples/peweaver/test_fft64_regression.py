...
try:
    ...
    from .fft64_regression import load_fixture, load_vector_file, run_regression
...
FFT64_REGRESSION_MODULE = run_regression.__module__


class Fft64ReferenceFixtureTests(unittest.TestCase):
    ...
    def test_reference_fixture_stays_out_of_the_formal_corpus(self):
        ...
        self.assertNotIn("halo_fft64_reference", names)
    ...
    def test_upstream_vectors_parse_to_64_samples(self):
        ...
        for vector_name in provenance["vector_files"]:
            ...
            self.assertEqual(len(pairs), 64, vector_name)
    ...
    def test_third_party_files_match_recorded_provenance_hashes(self):
        ...
        self.assertEqual(set(recorded), {"FFT64.v", "FFT128.v", "SdfUnit.v", "SdfUnit2.v",
                                         "Butterfly.v", "DelayBuffer.v", "Multiply.v",
                                         "Twiddle64.v", "Twiddle128.v", "LICENSE"})
    ...
    @patch(f"{FFT64_REGRESSION_MODULE}.resolve_verilator", return_value=None)
    ...
    def test_missing_fixture_directory_fails_closed(self):
        result = run_regression(ROOT / "benchmarks" / "does_not_exist_fft64")
        self.assertEqual(result.status, "tool_error")
        self.assertFalse(result.passed)
